...
# Declare catch2 dependency details. We will explicitly require 2.13.9 version this
# time.
externalproject_add(
    catch2_download
...
set(CATCH2_INCLUDE_DIR
    ${CMAKE_CURRENT_BINARY_DIR}/catch2_download-prefix/src/catch2_download/single_include
)

# Since the include directory of the catch2 is not available at the configure stage
# (ExternalProject_Add retrieves and builds the dependency at build stage) we'll just
# go ahead and create the include directory, so we can list that directory in
...
# Make the library dependent on the ExternalProject target, so CMake knows that in
# order to use this target, the ExternalProject target must be built first
add_dependencies(catch2 catch2_download)
# Add CATCH2_INCLUDE_DIR to INTERFACE_INCLUDE_DIRECTORIES of the catch2 target, so
# the libraries depending on catch2 will automatically be able to include catch2
# headers.
