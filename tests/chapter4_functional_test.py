...
class TestChapter4:
    ...
    def test_install_check_files_exists(self):
        ...
        # Example 1
        assert file_exists(
            f"{self.temporary_install_root}/bin/ch4_ex01_executable{PLATFORM_EXECUTABLE_EXTENSION}"
        )
        # Example 2
        ...
        assert (
            len(
                glob.glob(
                    f"{self.temporary_install_root}/lib/**/{PLATFORM_STATIC_LIBRARY_PREFIX}ch4_ex02_static{PLATFORM_STATIC_LIBRARY_EXTENSION}",
                    recursive=True,
                )
            )
            > 0
        ...
        assert file_exists(
            f"{self.temporary_install_root}/include/chapter4/ex02/lib.hpp"
        )
        # Example 3
        assert file_exists(
            f"{self.temporary_install_root}/bin/chapter4_greeter_content"
        )
        ...
        assert not file_exists(
            f"{self.temporary_install_root}/var/dir2/chapter1_hello.hpp"
        )
        assert file_exists(f"{self.temporary_install_root}/var/dir3/asset4")
        assert not file_exists(
            f"{self.temporary_install_root}/var/dir3/bin/goodbye.dat"
        )
        # Example 5
        assert (
            len(
                glob.glob(
                    f"{self.temporary_install_root}/lib/**/{PLATFORM_STATIC_LIBRARY_PREFIX}ch4_ex05_lib{PLATFORM_STATIC_LIBRARY_EXTENSION}",
                    recursive=True,
                )
            )
            > 0
        ...
        assert file_exists(
            f"{self.temporary_install_root}/include/chapter4/ex05/lib.hpp"
        )
        assert file_exists(
            f"{self.temporary_install_root}/cmake/ch4_ex05_lib-config.cmake"
        )
        assert file_exists(
            f"{self.temporary_install_root}/cmake/ch4_ex05_lib-config-version.cmake"
        )
    ...
    def test_install_run_binaries(self):
        ...
        assert run_command(
            "python3", f"{self.temporary_install_root}/bin/chapter4_greeter"
        )
    ...
    def test_ex05_consumer(self):
        ...
        ex05_source_directory = str(
            get_chapter4_root_directory().joinpath("ex05_consumer").absolute()
        )
        assert run_cmake(
            "-S",
            ex05_source_directory,
            "-B",
            ex05_temporary_build_root,
            f"-DCMAKE_INSTALL_PREFIX:STRING={self.temporary_install_root}",
        )
        assert run_cmake(
            "--build",
            ex05_temporary_build_root,
...
class TestChapter4Example6:
    ...
    source_directory = str(
        get_chapter4_root_directory().joinpath("ex06_pack").absolute()
    )
    ...
    def test_install_check_files(self):
        """Check if install step has installed the all files that install() commands intend to install"""
        assert file_exists(
            f"{self.temporary_install_root}/bin/ch4_ex06_executable{PLATFORM_EXECUTABLE_EXTENSION}"
        )
        # use globbing to account for platform-tuple suffixes i.e. lib/x64_86-linux-gnu etc
        assert (
            len(
                glob.glob(
                    f"{self.temporary_install_root}/lib/**/{PLATFORM_STATIC_LIBRARY_PREFIX}ch4_ex06_library{PLATFORM_STATIC_LIBRARY_EXTENSION}",
                    recursive=True,
                )
            )
            > 0
        ...
        assert file_exists(
            f"{self.temporary_install_root}/include/chapter4/ex06/lib.hpp"
        )
    ...
    def test_pack(self):
        ...
        assert run_cpack(
            "--config",
            f"{self.temporary_build_root}/CPackConfig.cmake",
            *CPACK_COMMAND_MULTICONFIG_ARGS,
            "-G",
            "TGZ",
            "-B",
            f"{self.temporary_build_root}/pak",
        )
    ...
    def test_pack_check_files(self):
        ...
        for system_name in ["Linux", "win32", "win64", "Darwin"]:
            if file_exists(
                f"{self.temporary_build_root}/pak/ch4_ex06_pack-1.0-{system_name}.tar.gz"
            ):
                return True
